...
b12=[0 10 30]*e*(1D+45)*(1D+6)
for j=1:3
    V1(:,j)=(0.5*k*(r.*r)+((1/3)*b12(j)*r.*r.*r))/e
//    V1(:,j)=(0.5*k*(r(j).*r(j))+((1/3)*b12(j)*r(j).*r(j).*r(j)))/e
end
show_window(4) // potential plot>>>>>>>>>>>>>>>>>>>>>>>>>
plot(r'*(1d+15),[V1(:,1) V1(:,2) V1(:,3)],'linewidth',2)
title("<< Potential vs r graph >>",'color','blue','Fontsize','4')
xlabel('Position (r) (in fm) ------------>>>>','color','brown','Fontsize','4')
ylabel('Potential (V) in eV ------------>>>>','color','brown','Fontsize','4')
legend("b = 0","b = 10","b = 30",2)
xgrid()
A=eye(n-2,n-2)
v=diag(V(2:n-1))
//disp(A)
...
if b==0 then
    disp("    Eth(eV)       Ecomp(eV) for b=0J/m^3    ")
    disp(" "+string(Eth(1:5))+"      "+string(Z(1:5)))
    else
    disp("Extracted Eigen values(eV) for b(in J/m^3) : ",Z(1:5))
end
...
disp(string(Z(1:4))+"        "+string(E(1:counter)))
//Comparison between theo & Calculated value
//Z=Z(1:counter)

show_window(1)
    plot(rnew*(1D+15),-u_r(:,1),'r')
...
show_window(3)
    plot(rnew*(1D+15),-R(:,1),'g')
    title("R(r) vs r plot ",'color','brown','Fontsize','5','Fontname','2')
    xlabel('r (fm) ------> ','color','brown','Fontsize','4','Fontname','4')
